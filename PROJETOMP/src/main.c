	...
	*botao_editar_ativado, *botao_sair_desativado, *botao_sair_ativado, *background, *easter_egg, *botao_show_ativado, *botao_show_desativado;
	...
	while (done == 0) {// Enquanto não concluir
		while(SDL_PollEvent(&event)){// loop de eventos
			...
			if (event.type == SDL_MOUSEBUTTONDOWN && (! Is_on(destino_botao_editar, button_width,button_height,x,y))  ) {// Se o usuário clicou
				int identificador;
				SDL_Quit();
				...
				printw("Qual o ID da tarefa que deseja editar?:\n ");
				
				scanw("%d", &identificador);
			
				
				
				grafo = editaTarefa(grafo, identificador);
				clear();
			...
			if ( event.type == SDL_MOUSEBUTTONDOWN && (! Is_on(destino_botao_adicionar, button_width,button_height,x,y))  ) {// Se o usuário clicou
				SDL_Quit();
